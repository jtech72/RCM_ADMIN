        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {cards.map((card, index) => {
                ...
                return (
                    <div
                        key={index}
                        className={`bg-gradient-to-br ${card.bgGradient} backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6 py-3 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 group cursor-pointer`}
                        style={{
                        ...
                        <div className="flex items-center justify-between mb-0">
                            <div className="flex items-center">
                                <div className={`flex-shrink-0 bg-gradient-to-r ${card.gradient} rounded-xl p-3 shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                                    <IconComponent className="h-3 w-4 text-white" />
                                </div>
                                <div className="ml-4 ">
                                    <h3 className="text-sm font-semibold my-0 text-gray-600 uppercase tracking-wide">
                                        {card.title}
                                    </h3>
                                    <p className={`text-md font-bold my-0 ${card.textColor} group-hover:scale-105 transition-transform duration-300`}>
                                        {card.value.toLocaleString()}
                                    </p>
                                </div>
                            </div>
                            {/* <div className={`flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium ${card.changeType === 'positive'
